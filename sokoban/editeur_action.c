...
int test2(PLATEAU P, int x, int y)
{				//x et y sont des variables qui correspondent au déplacement du perso (x pour les déplacement gauche-droite, y pour haut-bas)
	if (P.la_case[P.perso.x + x][P.perso.y + y].mode == MUR)
		return FALSE;
...
PLATEAU deplacer_select(PLATEAU P, int fleche, int caisse_select,
			int *nb_deplacement)
{
	//cas 1: la caisse à droite a été sélectionnée
	if (caisse_select == SELECT_DROITE) {
		if (fleche == FLECHE_HAUT || fleche == FLECHE_BAS)
			return P;	//empeche le personnage d'aller en haut et en bas
		if (fleche == FLECHE_DROITE) {
			if (*nb_deplacement == 0)
				return P;	//test si le joueur peut revenir en arrière
			P = deplacer(P, 1, 0);
	...
	if (caisse_select == SELECT_HAUT) {
		if (fleche == FLECHE_GAUCHE || fleche == FLECHE_DROITE)
			return P;
		if (fleche == FLECHE_HAUT) {
			if (*nb_deplacement == 0)
				return P;
...
//calcule alétoirement la direction du personnage
int alea_fleche(PLATEAU P, int caisse_select)
{
	int fleche = alea_int(50) % 4;

	if (caisse_select == 0) {
		if (fleche == 0 && test2(P, 0, 1)) {
			fleche = FLECHE_HAUT;
			return fleche;
		} else
			fleche++;
...
//calcule combien il y a de caisses adjacentes au personnage
int test_caisse_adj(PLATEAU P)
{
	int nb = 0;

	if (P.la_case[P.perso.x][P.perso.y + 1].mode == CAISSE)
		nb++;
	if (P.la_case[P.perso.x][P.perso.y - 1].mode == CAISSE)
		nb++;
	if (P.la_case[P.perso.x + 1][P.perso.y].mode == CAISSE)
		nb++;
	if (P.la_case[P.perso.x - 1][P.perso.y].mode == CAISSE)
		nb++;
	return nb;
...
//recherche l'emplacement de la caisse et renvoi sa selection
...
//appelle la fonction de recherche d'une caisse en fonction du nombre de caisse autour du personnage
int alea_select(PLATEAU P, int nb_caisse)
{
	if (nb_caisse == 1)
		return rechercher_caisse(P, 1);
	if (nb_caisse == 2)
		return rechercher_caisse(P, alea_int(2) + 1);
	if (nb_caisse == 3)
		return rechercher_caisse(P, alea_int(3) + 1);
	return rechercher_caisse(P, alea_int(4) + 1);
}

//fonction de déplacement au hasard
//calcule aléatoirement si le personnage bouge ou s'il sélectionne ou déselectionne une caisse
	...
	for (N = 0; N < 300; N++) {
		if (caisse_select != 0) {
			...
			if (alea != 0) {
				...
				P = deplacer_select(P, fleche, caisse_select,
						    &nb_deplacement);
			} else {
				caisse_select = 0;
				nb_deplacement = 0;
