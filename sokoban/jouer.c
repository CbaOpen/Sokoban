...
int test(PLATEAU P, int x, int y)
{				//x et y sont des variables qui correspondent au déplacement du perso (x pour les déplacement gauche-droite, y pour haut-bas)
	...
	if (P.la_case[P.perso.x + x][P.perso.y + y].mode == CAISSE) {
		if (P.la_case[P.perso.x + x * 2][P.perso.y + y * 2].mode ==
		    VIDE)
			return 2;
		return 0;
	}
...
int gestion_clic(POINT p, INFO * I)
{
	...
	if (p.x < LARG_BOUTON)
		return UNDO;
	if (p.x < (LARG_BOUTON * 2))
		return REDO;
	if (p.x < (LARG_BOUTON * 3))
		return INIT;
...
PLATEAU gestion_action_bouton(PLATEAU P, int bouton, INFO * I, PILE * pileU,
			      PILE * pileR)
{
	char str_niv[10];
...
PLATEAU fait_action(PLATEAU P, INFO * I, PILE * pileU, PILE * pileR)
{
	...
	if (event == EST_TOUCHE)
		bouton = gestion_touche(touche, I);
	if (event == EST_CLIC)
		bouton = gestion_clic(p, I);
