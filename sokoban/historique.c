...
void empiler(PILE * pile, struct coordonnees p, int caisse_dep, int direction,
	     int test)
{
	Element *nouveau = malloc(sizeof(Element));
	if (pile == NULL || nouveau == NULL) {
		printf
		    ("probleme d'allocation memoire ou le pointeur pile pointe vers NULL au lieu d'une struct pile");
		exit(EXIT_FAILURE);
	}
...
HISTORIQUE depiler(PILE * pile)
{
	...
	if (pile == NULL) {
		printf
		    ("le pointeur pile pointe vers NULL au lieu d'une struct pile");
		exit(EXIT_FAILURE);
	}
...
//dépile tout jusqu'à ce que la valeur test soit différente de 1
