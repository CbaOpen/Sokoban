...
void affiche_caisse(int x, int y, int etat)
{
	...
	if (etat == RANGEMENT)
		draw_fill_rectangle(bg, hd, COUL_CAISSE_RANGE);
	else
		draw_fill_rectangle(bg, hd, COUL_CAISSE);
}
...
void affiche_rangement(int x, int y)
{
	...
	bas.x = x * TAILLE_CASE;
	bas.y = y * TAILLE_CASE;
	haut.x = (x + 1) * TAILLE_CASE;
	haut.y = (y + 1) * TAILLE_CASE;
...
void affiche_plateau(PLATEAU P)
{
	...
	for (x = 0; x < N_LARG; x++) {
		for (y = 0; y < N_HAUT; y++) {
			if (P.la_case[x][y].mode == MUR)
				affiche_mur(x, y);
			if (P.la_case[x][y].etat == RANGEMENT)
				affiche_rangement(x, y);
			if (P.la_case[x][y].mode == CAISSE)
				affiche_caisse(x, y, P.la_case[x][y].etat);
...
void affiche_un_bouton(int x, char *texte, int la_selection)
{
	...
	bg.x = x * LARG_BOUTON;
	bg.y = HAUT_FENETRE - HAUT_BOUTON;
	hd.x = bg.x + LARG_BOUTON;
	hd.y = HAUT_FENETRE;
	centre.x = bg.x + LARG_BOUTON / 2;
	centre.y = HAUT_FENETRE - HAUT_BOUTON / 2;
...
void affiche_info_jeu(char *nom_fichier, char *niveau, int coups_joues)
{
	...
	bg.x = 0;
	bg.y = HAUT_FENETRE - 2 * HAUT_BOUTON;
	hd.x = LARG_FENETRE;
	hd.y = HAUT_FENETRE - HAUT_BOUTON;
	centre.x = 3 * N_LARG;
	centre.y = hd.y - HAUT_BOUTON / 2;
	...
	centre.x = centre.x + 3 * N_LARG;
	centre.y = centre.y + HAUT_BOUTON / 3;
